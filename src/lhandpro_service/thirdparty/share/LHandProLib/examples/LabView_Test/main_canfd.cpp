...
// 机械手节点。此 DLL 内部已封装 CANFD 通信细节，测试程序无需处理回调。
//
// 主要流程包括:
// Main flow includes:
// 1. 输入 CANFD 设备索引
// 2. 调用 lv_init 初始化（内部完成设备打开、SDK 创建、后台线程启动）
// 3. 读取自由度，完成使能与回零
// 4. 执行循环运动示例
// 5. 调用 lv_close 清理资源
// ============================================================================
...
int main() {
  ...
  // 1. 扫描并选择 CANFD 设备
  // ---------------------------------------------------------------------------
  std::cout << "Using LHandProLib_LV.dll (CANFD communication)."
            << std::endl
            << std::endl;

  std::cout << "Scanning CANFD devices..." << std::endl;
  int dev_count = lv_canfd_scan_device_count();
  if (dev_count <= 0) {
    std::cerr << "No CANFD device found." << std::endl;
    wait_before_exit();
    return -1;
  }
  ...
  for (int i = 0; i < dev_count; ++i) {
    char info[64] = {0};
    if (lv_canfd_get_device_string(i, info, sizeof(info)) == 0) {
      std::cout << "  [" << i << "] " << info << std::endl;
    }
  }

  const int device_index = select_number_in_range(
      "Please select the CANFD device index", 0, dev_count - 1);

  // ---------------------------------------------------------------------------
  // 2. 调用 lv_init 初始化
  //    内部完成：打开 CANFD 设备、创建 SDK 实例、注册回调、启动后台线程
  // ---------------------------------------------------------------------------
  std::cout << "Initializing with lv_init(CANFD, device=" << device_index
            << ", node=" << kNodeId << ")..." << std::endl;

  int init_ret = lv_init(LV_COMM_CANFD, device_index, kNodeId);
  if (init_ret != 0) {
