...
// 机械手节点。此 DLL 内部已封装 RS485 通信细节，测试程序无需处理回调。
//
// 主要流程包括:
// Main flow includes:
// 1. 输入 RS485 串口索引
// 2. 调用 lv_init 初始化（内部完成串口打开、SDK 创建、读取回调注册）
// 3. 读取自由度，完成使能与回零
// 4. 执行循环运动示例
// 5. 调用 lv_close 清理资源
// ============================================================================
...
int main() {
  ...
  // 1. 扫描并选择 RS485 串口
  // ---------------------------------------------------------------------------
  std::cout << "Using LHandProLib_LV.dll (RS485 communication)."
            << std::endl
            << std::endl;

  std::cout << "Scanning RS485 serial ports..." << std::endl;
  int dev_count = lv_rs485_scan_device_count();
  if (dev_count <= 0) {
    std::cerr << "No RS485 serial port found." << std::endl;
    wait_before_exit();
    return -1;
  }

  std::cout << "Found " << dev_count << " port(s):" << std::endl;
  for (int i = 0; i < dev_count; ++i) {
    char info[256] = {0};
    if (lv_rs485_get_device_string(i, info, sizeof(info)) == 0) {
      std::cout << "  [" << i << "] " << info << std::endl;
    }
  }

  const int device_index = select_number_in_range(
      "Please select the RS485 serial port index", 0, dev_count - 1);

  // ---------------------------------------------------------------------------
  // 2. 调用 lv_init 初始化
  //    内部完成：打开串口、创建 SDK 实例、注册回调、启动读取线程
  // ---------------------------------------------------------------------------
  std::cout << "Initializing with lv_init(RS485, device=" << device_index
            << ", node=" << kNodeId << ")..." << std::endl;

  int init_ret = lv_init(LV_COMM_RS485, device_index, kNodeId);
  if (init_ret != 0) {
