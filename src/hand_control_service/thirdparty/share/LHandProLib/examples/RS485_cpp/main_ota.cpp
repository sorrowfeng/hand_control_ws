// ============================================================================
// 文件说明:
// 该示例演示如何通过 C++ SDK 在 RS485 串口下进行固件升级 (OTA)。
// 连接串口后输入固件文件路径开始升级。
// ============================================================================
...
constexpr int kOtaResponseTimeoutMs = 5000;
constexpr int kOtaMaxRetryCount = 5;
constexpr int kOtaRetryIntervalMs = 1000;
...
int run_ota_upgrade(SDK_NS::SDK_CLASS& lib,
  ...
  const StepDef steps[] = {
      ...
      {4, 0x53, 0x03, "Prepare to burn", 0x5A},
  };
...
int main() {
  ...
  // ── 1. Scan and select serial port ────────────────────────────────────
  std::cout << "RS485 OTA Firmware Upgrade Test" << std::endl << std::endl;
  ...
  for (size_t i = 0; i < port_names.size(); ++i) {
    std::cout << "Input " << i << " ---- port " << port_names[i] << std::endl;
  }
  ...
  // ── 4. Get firmware path and run OTA ──────────────────────────────────
  {
