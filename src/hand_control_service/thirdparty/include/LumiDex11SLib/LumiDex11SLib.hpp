#ifndef LUMIDEX11SLIB_HPP
#define LUMIDEX11SLIB_HPP
...
#define SDK_CPP_NAMESPACE lumidex
#endif

#ifndef SDK_CPP_CLASS_NAME
#define SDK_CPP_CLASS_NAME LumiDex11SLib
#endif

#ifndef SDK_CPP_PRIVATE_CLASS_NAME
#define SDK_CPP_PRIVATE_CLASS_NAME LumiDex11SLibPrivate
#endif
...
class LumiDex11SLibPrivate;

/// @brief LumiDex11SLib类 / LumiDex11SLib class
class SDK_API LumiDex11SLib {
  L_DECLARE_PRIVATE(LumiDex11SLib)
 public:
  ...
  /// @param callback_impl 函数指针(必须保证该 std::function 对象生命周期长于 LumiDex11SLib 实例)
  ///        callback_impl: Function pointer (must ensure the std::function object lifetime is longer than LumiDex11SLib instance)
  /// @example
  ///           // 1. 定义EtherCAT主站对象
  ///           // 1. Define EtherCAT master object
  ///           auto ec_master_ = std::make_shared<EthercatMaster>();
  ...
  ///             return ec_master_->setOutputs(data, size);
  ///           };
  ///           // 3. 使用std::function包装(必须保证该 std::function 对象生命周期长于 LumiDex11SLib 实例)
  ///           // 3. Use std::function wrapper (must ensure the std::function
  ///           // object lifetime is longer than LumiDex11SLib instance)
  ...
  ///           };
  ///           // 3. 使用std::function包装(必须保证该 std::function 对象生命周期长于 LumiDex11SLib 实例)
  ///           // 3. Use std::function wrapper (must ensure the std::function object lifetime is longer than LumiDex11SLib instance)
  ///           std::function<bool(const unsigned char*, unsigned int)> func = send_func;
  ///           // 4. 处理CANFD发送数据的回调
